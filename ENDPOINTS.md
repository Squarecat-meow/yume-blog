...
- Access 검증은 Cloudflare Access(엣지 차단)와 `lib/admin-guard.ts`(앱 내 인가) 두 계층입니다. 앱 전체를 한 번에 막아주는 미들웨어 계층은 없으므로, **관리자 데이터에 닿는 모든 진입점이 각자 가드를 호출해야 합니다.**
  - 라우트 핸들러: `withAdmin()`으로 감쌉니다. 검증을 통과해야만 핸들러가 실행되고 신원이 세 번째 인자로 넘어옵니다. 세밀한 제어가 필요하면 `requireAdminApi()`를 직접 씁니다.
  - 페이지·서버 액션: `requireAdmin()`을 호출합니다. 서버 액션은 공개 API와 동일하게 취급합니다.
- `app/admin/layout.tsx`에서 한 번에 처리하려 하지 마세요. Partial Rendering 때문에 하위 세그먼트가 그대로 실행되고 RSC 페이로드에 데이터가 실립니다.
- 관리자 페이지에는 `export const dynamic = 'force-dynamic'`을 지정합니다. 프리렌더된 응답이 캐시에서 나가면 검증 코드가 실행되지 않습니다.
- D1 조회 계층을 만들 때 조회 함수 안에서 `requireAdmin()`을 호출하면, 라우트에서 가드를 깜빡해도 데이터가 나가지 않습니다. 가능하면 이 방식을 씁니다.
